...
function processLine() {
    ...
    .then(data => {
        ...
        // Remove any existing download buttons before adding new ones
        const existingButtons = document.getElementById('download-container');
        if (existingButtons) {
            existingButtons.remove();
        }

        // Create a new container for download buttons
        ...
        // Check if the backend response includes file paths and create buttons
        if (data.output_video_path) {
            downloadContainer.appendChild(createDownloadButton(data.output_video_path, 'Download Processed Video'));
        }
        if (data.count_report_path) {
            downloadContainer.appendChild(createDownloadButton(data.count_report_path, 'Download Count Report'));
        }
        if (data.environmental_report_path) {
            downloadContainer.appendChild(createDownloadButton(data.environmental_report_path, 'Download Environmental Report'));
        }

        // Add download buttons to the body if any exist
    ...
    .finally(() => {
        // Hide spinner ONLY AFTER the download buttons are added
